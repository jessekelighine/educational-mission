...
rss_file="rss.xml"
posts_dir="_posts"
latest_post_file="$(ls "$posts_dir"/*.md 2>/dev/null | sort | tail -n 1)"
latest_title="$(awk -F': ' '/^title: / {print $2; exit}' "$latest_post_file" | sed -e "s/^['\"]//" -e "s/['\"]$//")"
[[ -z "$latest_title" ]] && latest_title="$(basename "$latest_post_file" .md)"
...
echo "$arrow_echo Latest post in $(basename "$posts_dir"): $latest_title"
